...
DefaultSpecDatasetOps = {
    ...
    # A value being a power of 2 (=8,16,32,64,128,256,512,etc.) should be chosen to not run into odd numbers.
    "n_freq_bins": 256,
    "fmin": 0,
    "fmax": 2500,
...
"""
# inherit from object to be compatiable with python 2, python3 is no longer needed
...
"""
Computes the CSV Split in order to prepare for randomly partitioning all data files into a training, validation, and test
corpus. 
Audio files of a given tape are stored only in one of the three partitions.
...
class CsvSplit(object):
    ...
    def __init__(
        ...
        self.splits = defaultdict(list) # defaultdict(list): pass list to .default_factory
        self._logger = Logger("CSVSPLIT")

    """
    Return split for given partition. If there is already an existing CSV split return this split.
    If there is no split yet, generate a new CSV split.
    """
    def load(self, split: str, files: List[Any] = None):
        ...
        if self.splits[split]: # return either an empty list (False) or a non-empty list
...
class DatabaseCsvSplit(CsvSplit):
    valid_years = set(range(1950, 221231)) # modified to accomodate YYMMDD structure of the Rek. community data
...
Called by main.py during training
"""
...
Called by predict.py

---------
time-bins (# of time_frames) is determined by sample rate, sequence length and hop-size:
time_bins = (Sequence_Len/1000) * (Sampling_Rate/Hop_Size) --> used in main.py
e.g. Sampling Rate = 44100 Hz, Sequence Length = 1280ms, Hop-Size = 441 Samples
time_bins = (1280/1000)*(44100/441) = 128
time_bins could be adjusted by adjusting Sampling Rate, Hop, or Sequence Length.

alternative: time_bins = seq_len (s) / (hop_size/SR)

# seq_len: in ms; (sequence_len in later code);determined by the average length/duration of the target signals; orca: 1.28s
input shape: (128, 256) (W,H)
---------
"""
class StridedAudioDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.n_frames = self.audio.shape[1] # total num of samples in the audio (transposed mono)
    ...
    def __len__(self):
        return max(int(ceil((self.n_frames + 1 - self.sequence_len) / self.hop)), 1) # +1: to enable the inclusion of beginning of last seq
...
# this class is not called and has been removed in orcaspot
class StridedAudioDatasetVis(torch.utils.data.Dataset):
    def __init__(
        self,
        file_name,
        sequence_len: int,
